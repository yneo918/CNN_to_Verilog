module pack_conv
	...
	genvar i,j,k;
	assign z = z_L1;
	conv2d_L1_top #(.WIDTH(WIDTH), .W(28), .H(28), .CIN(1), .COUT(6), .F(5)) conv_l1(.x(x_L1), .z(z_L1));
	generate
		for (i = 0; i < 6; i++) begin
			for (j = 0; j < 24; j++) begin
				for (k = 0; k < 24; k++) begin
					assign x_L2[i] = z_L1[i][j][k][WIDTH*2+$clog2(25)-1:WIDTH+$clog2(25)+1];
				end
			end
		end
	endgenerate
	maxpool_L2_top #(.WIDTH(WIDTH), .W(24), .H(24), .C(6)) maxpool_L2(.x(x_L2), .z(z_L2));
	conv2d_L3_top #(.WIDTH(WIDTH), .W(12), .H(12), .CIN(6), .COUT(16), .F(5)) conv_l3(.x(x_L3), .z(z_L3));
	generate
		for (i = 0; i < 16; i++) begin
			for (j = 0; j < 8; j++) begin
				for (k = 0; k < 8; k++) begin
					assign x_L4[i] = z_L3[i][j][k][WIDTH*2+$clog2(150)-1:WIDTH+$clog2(150)+1];
				end
			end
		end
	endgenerate
	maxpool_L4_top #(.WIDTH(WIDTH), .W(8), .H(8), .C(16)) maxpool_L4(.x(x_L4), .z(z_L4));
	assign z = z_L4;
